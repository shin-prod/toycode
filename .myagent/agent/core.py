...
from agent.stream import print_stream, print_tool_call, print_tool_result
...
class Agent:
    ...
    def run(self, user_input: str) -> str:
        ...
        while loop_count < max_loops:
            ...
            if finish_reason == "tool_calls" and tool_calls_data:
                ...
                # ツールを実行して結果を履歴に追加
                tool_results = self._execute_tools(tool_call_objects)
        ...
        if self._settings.stream:
            print(f"\n\033[31m{warning_msg}\033[0m", flush=True)
    ...
    def _call_stream(self) -> tuple[str, str, dict]:
        """ストリーミングで LLM を呼び出し、テキスト表示しながら結果を返す。
    ...
    def _execute_tools(self, tool_calls: list[ToolCallData]) -> list[dict]:
        """ツールを実行して結果リストを返す。
